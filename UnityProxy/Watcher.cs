...
namespace UnityProxy
{
                        ...
                        using (StreamReader reader = new StreamReader(stream))
                        {
                            ...
                            if (!Failed)
                            {
                                ...
                                foreach (var line in lines)
                                {
                                    ...
                                    {
                                        Failed = true;
                                        Console.WriteLine($"##teamcity[buildProblem description='{line.Replace("'", "\"")}']");
                                        break;
                                    }
                                }
                            }
    ...
    public static class ErrorFinder
    {
        public static bool IsError(string line)
        {
            foreach (var pattern in new[] { "Exception: ", "Build failure!", "Compilation failed", "ERROR:", "): error CS", "Error building Player" })
            {
